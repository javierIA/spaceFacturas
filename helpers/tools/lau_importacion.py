...
def extract_pieces_data(pieces, pieces_clean):
    ...
    rows = data[data.str.contains('(?:[\d|,|\.]+)\s*(?:[A-Z]+)\s+\$?(?:[\d|,|\.]+)\s*$')]
    parts = rows.str.extract('([\d|,|\.]+)\s*([A-Z]+)\s*\$?([\d|,|\.]+)\s*$')
...
def extract_from_pdf(filename):
    ...
    tables_full = use_appropiate_template(filename)
...
def extract_data_lau_import(filename):
